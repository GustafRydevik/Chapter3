...
ll.grid<-outer(a.range,b.range,function(X,Y){
  sapply(1:length(X),function(x){
    val<-tsi.linear.ll(pars =c(log(X[x]),log(1e-6+Y[x]+X[x]/c)),
                       data = plot.data,censorLimit. = c,popsize =10000/(c*(a+b.abs*c/2)),
...
ggplot(transform(melt((ll.grid)),ll.quantile=1-ecdf(value)(value)),
                 aes(x=Var1,y=Var2))+
  geom_tile(aes(fill=(ll.quantile)))+
  scale_alpha_manual(values=c(0,1),guide="none")+
  scale_fill_gradient(low="red", high="green")+
  geom_point(x=a,y=b.abs)+
  geom_point(x=max.coord[1],y=max.coord[2])+
  ...
  ggtitle("Likelihood for different values of alpha and beta given data.
          \nLine indicates line of same average incidence")

hist(plot.data,breaks=0:11,freq=FALSE)
lines(0:11,lineardist(time = 0:11,
                      alpha = max.coord[1],beta=max.coord[2],chain = 1)*10/sum(lineardist(time = seq(0,10,by=0.1),
                                                                                          alpha = max.coord[1],beta=max.coord[2],chain = 1)),col="blue")
lines(0:11,lineardist(time = 0:11,alpha = a,beta=b.abs,chain = 1)*10/sum(lineardist(time = seq(0,10,by=0.1),
                                                                                alpha = a,beta=b.abs,chain=1)),col="red")
